...
Frontend körs på Next.js 16 med App Router och React 19. Komponenter renderas server-side för initial load, sedan hanterar React interaktivitet på klienten. Routing sker via filsystembaserade routes i `src/app/`. Applikationen är en monolitisk deployment där frontend och backend-logik kombineras i samma build.

Backend består av Next.js API routes (`src/app/api/`) som körs som serverless functions. Databasaccess hanteras via Prisma ORM mot PostgreSQL i Supabase. Två connection strings används: `DATABASE_URL` för pooled queries (port 6543) och `DIRECT_URL` för migrations (port 5432). Autentisering sker på infrastrukturnivå via Azure SWA med Google OAuth som provider.

TypeScript används genomgående med strict mode. Prisma genererar typer från databasschemat, vilket ger compile-time safety för alla databasoperationer. Input-validering implementeras med Zod-scheman som både validerar runtime-data och genererar TypeScript-typer. Samma schema används på klient och server för att garantera kontrakt-konsistens.

Styling implementeras i en enda global SCSS-fil (`src/styles/theme.scss`, 1021 rader) som innehåller design tokens, komponentmönster och utility classes. WCAG AAA-compliant färgkontraster och responsiv design är inbyggt. Inga komponent-specifika CSS-filer används.

Azure Static Web Apps i Hybrid Mode kombinerar statisk webbhosting med serverless compute i en enda deployment-enhet. Statiska assets (bundlade JS, CSS, bilder från `.next/static/`) distribueras till Azures globala CDN-noder för låg latens. Dynamiska requests (API routes, SSR-rendered pages) routas till en Node.js 20 serverless runtime-instans som startas on-demand per request. Build-processen genererar både statiska filer och en standalone server-bundle som Azure kör i isolation. Detta skiljer sig från traditionell Static Web App-mode (endast statiska filer) och App Service (dedikerad server-instans).
